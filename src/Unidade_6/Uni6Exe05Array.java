...
public class Uni6Exe05Array {
    ...
    private ArrayList<String> pergunta = new ArrayList<>();
    // private char[][] resposta = new char[2][5];
    private ArrayList<Character> resposta = new ArrayList<>();
    ...
    private String perguntas(int p){
        ...
        pergunta.add("Gosta de música sertaneja?");
        // pergunta[0][1] = "Gosta de futebol?";
        pergunta.add("Gosta de futebol?");
        // pergunta[0][2] = "Gosta de seriados?";
        pergunta.add("Gosta de seriados?");
        // pergunta[0][3] = "Gosta de redes sociais?";
        pergunta.add("Gosta de redes sociais?");
        // pergunta[0][4] = "Gosta de Oktoberfest?";
        pergunta.add("Gosta de Oktoberfest?");
        return pergunta.get(p);
    }
    ...
    private void resultado(){
        if(afinidade >= 15){
            ...
            System.out.println("Valem um encontro.");
    ...
    private void resposta(){
        for(int i = 0; i < 2; i++){
            ...
            for(int j = 0; j < 5; j++){
                ...
                resposta.add(sc.next().toUpperCase().charAt(0));
            }
        }
    }
    private void pontuacao(){
        for(int i = 0; i < 5; i++){
            if(resposta.get(i) == resposta.get(i+5)){
                afinidade += 3;
            }else if(resposta.get(i) == 'I' || resposta.get(i+5) == 'I'){
                afinidade += 1;
            }else if (resposta.get(i) != resposta.get(i+5)){
                afinidade -= 2;
            }
        }
    }
