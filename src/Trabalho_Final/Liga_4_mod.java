...
public class Liga_4_mod {
    ...
    public Liga_4_mod(boolean guiMode) {
        if (guiMode) {
            resetVar();
            iniciarTabuleiro();
            setPlayerColor('V');
    ...
    private int playerSelect(int id){
        if(id == 0){
            while (true) {
                ...
                int resultado = verificarColuna(0,0,0);
                if(resultado >= 0){
                    return resultado;
                }
            }
...
//Bot Joga
    private void botSelect(){
        ...
        do {
            ...
            tentativas++;
        } while (coluna >= 1 && coluna <= 7 && tabuleiro[0][coluna-1] != 'B' && tentativas < 20);
        if (coluna >= 1 && coluna <= 7 && tabuleiro[0][coluna-1] == 'B') {
    ...
    public void setPlayerColor(char cor) {
        if (cor == 'A') {
            playerCor[0] = 'A'; playerName[0] = "Azul";
            playerCor[1] = 'V'; playerName[1] = "Vermelho";
        } else {
    ...
    public int playPlayerColumn(int coluna) {
        if (coluna < 1 || coluna > 7) {
            return -1;
        }
        this.coluna = coluna;
        int linha = verificarColuna(0, 0, 0);
        if (linha >= 0) {
            jogadas++;
            atualizarVitoria(0);
        }
    ...
    public int playBot() {
        int tentativas = 0;
        int selectedColuna = -1;
        do {
            selectedColuna = (int)(Math.random() * 7) + 1;
            tentativas++;
        } while (selectedColuna >= 1 && selectedColuna <= 7 && tabuleiro[0][selectedColuna-1] != 'B' && tentativas < 20);
        if (selectedColuna >= 1 && selectedColuna <= 7 && tabuleiro[0][selectedColuna-1] == 'B') {
            this.coluna = selectedColuna;
            int linha = verificarColuna(1,0,0);
            if (linha >= 0) {
                jogadas++;
                atualizarVitoria(1);
            }
            return selectedColuna;
        }
        return -1;
    }

    private void atualizarVitoria(int playerId) {
        if (jogadas < 4) {
            return;
        }
        verificaVencedorLinha(playerId);
        verificaVencedorColuna(playerId);
        filtroDiagonal(playerId);
        if (!vencedor && isBoardFull()) {
            mensagemVitoria(-1);
        }
    }
